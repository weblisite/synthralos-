  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const messageHandler = (event: MessageEvent) => {
        ...
        if (event.data.type === 'NANGO_OAUTH_SUCCESS' || event.data.type === 'OAUTH_SUCCESS') {
          popupWindow?.close();
          setIsConnecting(false);
          toast.success(`Successfully connected to ${connectorName}`);
          onConnected?.();
          window.removeEventListener('message', messageHandler);
        } else if (event.data.type === 'NANGO_OAUTH_ERROR' || event.data.type === 'OAUTH_ERROR') {
          popupWindow?.close();
          setIsConnecting(false);
          toast.error(event.data.message || `Failed to connect to ${connectorName}`);
          window.removeEventListener('message', messageHandler);
        }
      };
