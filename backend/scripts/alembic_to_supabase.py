...
def get_migration_sql(revision: Optional[str] = None) -> str:
    """
...
def format_sql_for_supabase(sql: str) -> str:
    ...
    formatted_sql = re.sub(r'^\s*COMMIT\s*;?\s*$', '', formatted_sql, flags=re.MULTILINE)
...
def get_migration_name(revision: Optional[str] = None) -> str:
...
def save_sql_to_file(sql: str, migration_name: str, output_dir: Optional[Path] = None):
    """Save SQL to file."""
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"-- Generated from Alembic migration\n\n")
        f.write(sql)
...
def main():
    ...
    parser.add_argument(
        'revision',
        nargs='?',
        help='Specific Alembic revision ID (default: head)'
    )
    parser.add_argument(
        '--apply',
        action='store_true',
        help='Show MCP command to apply migration'
    )
    parser.add_argument(
        '--output-dir',
        type=str,
