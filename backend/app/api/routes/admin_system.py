...
def get_system_metrics(
    ...
    total_browser_sessions = session.exec(
        select(func.count(BrowserSession.id))
    ).one() or 0
    total_osint_streams = session.exec(
        select(func.count(OSINTStream.id))
    ).one() or 0
    total_code_tools = session.exec(
        select(func.count(CodeToolRegistry.id))
    ).one() or 0
