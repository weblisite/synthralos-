...
def list_available_agents(
    ...
    framework_configs = session.exec(
        select(AgentFrameworkConfig)
    ).all()
    
    # Also include frameworks that might not be in the database yet
    # Get all known framework names
    known_frameworks = {
        "agentgpt", "autogpt", "metagpt", "autogen", "archon",
        "crewai", "swarm", "camel_ai", "kush_ai", "kyro", "riona",
        "babyagi"
    }
    ...
    for framework_name in sorted(known_frameworks):
        ...
        try:
            router = default_agent_router
            if hasattr(router, '_framework_handlers') and framework_name in router._framework_handlers:
        ...
        result.append({
            ...
            "is_enabled": framework_config.is_enabled if framework_config else False,
            "is_available": is_available,
            "capabilities": capabilities,
            "config": framework_config.config if framework_config else {},
            "created_at": framework_config.created_at.isoformat() if framework_config and hasattr(framework_config, 'created_at') else None,
            "updated_at": framework_config.updated_at.isoformat() if framework_config and hasattr(framework_config, 'updated_at') else None,
        })
...
def list_agent_tasks(
    ...
    statement = statement.order_by(AgentTask.started_at.desc()).offset(skip).limit(limit)
...
def _generate_routing_reasoning(
    ...
    if task_requirements.get("framework"):
        reasons.append(f"Explicit framework preference: {task_requirements.get('framework')}")
    
    # Check multi-role requirement
    agent_roles = task_requirements.get("agent_roles", 1)
    if agent_roles > 1:
        reasons.append(f"Multi-role requirement ({agent_roles} roles) → {selected_framework}")
