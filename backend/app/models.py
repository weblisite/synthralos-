...
class User(UserBase, table=True):
    ...
    rag_indexes: list["RAGIndex"] = Relationship(back_populates="owner", cascade_delete=True)
    code_tools: list["CodeToolRegistry"] = Relationship(back_populates="owner", cascade_delete=True)
    code_sandboxes: list["CodeSandbox"] = Relationship(back_populates="owner", cascade_delete=True)
...
class Workflow(WorkflowBase, table=True):
    ...
    schedules: list["WorkflowSchedule"] = Relationship(back_populates="workflow", cascade_delete=True)
...
class WorkflowNode(SQLModel, table=True):
    ...
    node_type: str = Field(max_length=100)  # trigger, ai_prompt, http_request, code, etc.
...
class WorkflowExecution(SQLModel, table=True):
    ...
    execution_id: str = Field(unique=True, index=True, max_length=255)  # Custom execution ID
    status: str = Field(max_length=50)  # running, completed, failed, paused, waiting_for_signal
    ...
    logs: list["ExecutionLog"] = Relationship(back_populates="execution", cascade_delete=True)
    signals: list["WorkflowSignal"] = Relationship(back_populates="execution", cascade_delete=True)


class ExecutionLog(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    execution_id: uuid.UUID = Field(foreign_key="workflowexecution.id", nullable=False, ondelete="CASCADE")
...
class WorkflowSignal(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    execution_id: uuid.UUID = Field(foreign_key="workflowexecution.id", nullable=False, ondelete="CASCADE")
    signal_type: str = Field(max_length=100)  # connector_ready, human_input, webhook, etc.
...
class Connector(SQLModel, table=True):
    ...
    owner_id: uuid.UUID | None = Field(default=None, foreign_key="user.id", nullable=True, ondelete="CASCADE")
    is_platform: bool = Field(default=True)  # True = available to all users, False = user-specific
    created_by: uuid.UUID | None = Field(default=None, foreign_key="user.id", nullable=True, ondelete="SET NULL")
    
    versions: list["ConnectorVersion"] = Relationship(back_populates="connector", cascade_delete=True)


class ConnectorVersion(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    connector_id: uuid.UUID = Field(foreign_key="connector.id", nullable=False, ondelete="CASCADE")
    ...
    webhook_subscriptions: list["WebhookSubscription"] = Relationship(back_populates="connector_version", cascade_delete=True)


class WebhookSubscription(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    connector_version_id: uuid.UUID = Field(foreign_key="connectorversion.id", nullable=False, ondelete="CASCADE")
    trigger_id: str = Field(max_length=255)
    tenant_id: uuid.UUID = Field(nullable=False)
    endpoint_secret: str = Field(max_length=255)
    created_at: datetime = Field(default_factory=datetime.utcnow)
    
    connector_version: ConnectorVersion | None = Relationship(back_populates="webhook_subscriptions")
...
class AgentTaskLog(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    task_id: uuid.UUID = Field(foreign_key="agenttask.id", nullable=False, ondelete="CASCADE")
    level: str = Field(max_length=20)  # info, error, debug, warning
    message: str = Field(max_length=5000)
    timestamp: datetime = Field(default_factory=datetime.utcnow)
...
class RAGIndex(SQLModel, table=True):
    ...
    documents: list["RAGDocument"] = Relationship(back_populates="index", cascade_delete=True)
    queries: list["RAGQuery"] = Relationship(back_populates="index", cascade_delete=True)


class RAGDocument(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    index_id: uuid.UUID = Field(foreign_key="ragindex.id", nullable=False, ondelete="CASCADE")
...
class RAGSwitchLog(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    query_id: uuid.UUID | None = Field(default=None, foreign_key="ragquery.id", ondelete="SET NULL")
...
class RAGFinetuneDataset(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    job_id: uuid.UUID = Field(foreign_key="ragfinetunejob.id", nullable=False, ondelete="CASCADE")
...
class OCRDocument(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    job_id: uuid.UUID = Field(foreign_key="ocrjob.id", nullable=False, ondelete="CASCADE")
...
class ScrapeResult(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    job_id: uuid.UUID = Field(foreign_key="scrapejob.id", nullable=False, ondelete="CASCADE")
...
class BrowserSession(SQLModel, table=True):
    ...
    browser_tool: str = Field(max_length=100)  # Playwright, Puppeteer, Browserbase, etc.
    proxy_id: str | None = Field(default=None, max_length=255)
    status: str = Field(max_length=50)  # active, closed, error
    started_at: datetime = Field(default_factory=datetime.utcnow)
    closed_at: datetime | None = None
    
    actions: list["BrowserAction"] = Relationship(back_populates="session", cascade_delete=True)


class BrowserAction(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    session_id: uuid.UUID = Field(foreign_key="browsersession.id", nullable=False, ondelete="CASCADE")
...
class OSINTStream(SQLModel, table=True):
    ...
    signals: list["OSINTSignal"] = Relationship(back_populates="stream", cascade_delete=True)


class OSINTAlert(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    stream_id: uuid.UUID | None = Field(default=None, foreign_key="osintstream.id", ondelete="SET NULL")
...
class OSINTSignal(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    stream_id: uuid.UUID = Field(foreign_key="osintstream.id", nullable=False, ondelete="CASCADE")
...
class ToolUsageLog(SQLModel, table=True):
    ...
    tool_type: str = Field(index=True, max_length=100)  # connector, ocr, scraping, browser, etc.
...
class EventLog(SQLModel, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    event_type: str = Field(index=True, max_length=100)  # workflow_started, agent_switch, etc.
