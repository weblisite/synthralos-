...
class BrowserService:
    ...
    def _initialize_engines(self) -> None:
        ...
        try:
            ...
            logger.info("playwright not installed. Install with: pip install playwright && playwright install")
            self._browser_engines["playwright"] = {
                "name": "playwright",
                "is_available": False,
            }
    ...
    def create_session(
        ...
        if not proxy_id and auto_select_proxy:
            try:
                proxy = default_proxy_pool.get_proxy(
                    session=session,
                    country=automation_requirements.get("country") if automation_requirements else None,
                    proxy_type=automation_requirements.get("proxy_type") if automation_requirements else None,
                )
        ...
        logger.info(f"Created browser session: {browser_session.id} (Engine: {browser_tool})")
    ...
    def execute_action(
        ...
        try:
            ...
            logger.info(f"Executed browser action: {action_type} for session {session_id}")
    ...
    def list_sessions(
        ...
        statement = statement.order_by(BrowserSession.started_at.desc()).offset(skip).limit(limit)
    ...
    def _get_engine_client(self, engine: str) -> Any:
        ...
        if not client.get("is_available"):
            logger.warning(f"Browser engine '{engine}' not available. Falling back to Playwright or placeholder.")
    ...
    def _execute_browser_action(
        ...
        if engine_name in ["playwright", "puppeteer"]:
            return self._execute_playwright_action(client, browser_session, action_type, action_data)
        else:
            # Unknown engine, try Playwright as fallback
            if self._browser_engines.get("playwright", {}).get("is_available"):
                logger.info(f"Unknown engine '{engine_name}', falling back to Playwright")
        ...
        try:
            ...
            try:
                result = loop.run_until_complete(
                    self._async_execute_playwright_action(client, browser_session, action_type, action_data)
                )
            finally:
                loop.close()
            return result
    ...
    async def _async_execute_playwright_action(
        ...
        async with async_playwright() as p:
            ...
            context = await browser.new_context(
                user_agent=action_data.get("user_agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"),
            ...
            try:
                if action_type == "navigate":
                    ...
                    screenshot_bytes = await page.screenshot(full_page=action_data.get("full_page", False))
