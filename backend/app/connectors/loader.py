...
class ConnectorHotLoader:
    ...
    def load_connector(
        ...
        if version_id in self._loaded_modules and not force_reload:
            logger.debug(f"Connector {version_id} already loaded, returning cached module")
        ...
        if connector_version.wheel_url:
            ...
            try:
                ...
                logger.info(f"Loaded connector '{slug}' from installed package '{package_name}'")
    ...
    def _load_from_wheel(self, connector_version: ConnectorVersion) -> Any:
        ...
        try:
            ...
            logger.info(f"Successfully loaded connector '{slug}' version {connector_version.version}")
            return module
    ...
    def _load_module_from_path(self, module_path: Path, package_name: str) -> Any:
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                raise WheelLoadError(f"Could not create module spec for '{package_name}'")
    ...
    def invoke_action(
        ...
        if credentials:
            result = self.invoke_method(connector_version, method_name, credentials, input_data)
        else:
    ...
    def unload_connector(self, version_id: str) -> None:
        ...
        if version_id in self._temp_dirs:
            ...
            try:
                ...
                logger.debug(f"Cleaned up temp directory for connector version {version_id}")
            except Exception as e:
