...
class StorageService:
    ...
    def __init__(self):
        ...
        if not settings.SUPABASE_URL or not settings.SUPABASE_ANON_KEY:
            logger.warning("Supabase Storage not configured. SUPABASE_URL and SUPABASE_ANON_KEY required.")
    ...
    def upload_file(
        ...
        if not self.is_available or not self.client:
            raise StorageServiceError("Supabase Storage is not available. Check configuration.")
        ...
        try:
            ...
            result = {
                "path": file_path,
                "url": signed_url or public_url or f"{settings.SUPABASE_URL}/storage/v1/object/public/{bucket}/{file_path}",
            ...
            logger.info(f"File uploaded successfully: {bucket}/{file_path} ({len(file_bytes)} bytes)")
            return result
            
        except Exception as e:
            logger.error(f"Failed to upload file to {bucket}/{file_path}: {e}", exc_info=True)
    ...
    def download_file(
        ...
        try:
            ...
            if isinstance(file_data, bytes):
                ...
                return file_data.read() if hasattr(file_data, 'read') else bytes(file_data)
                
        except Exception as e:
            logger.error(f"Failed to download file from {bucket}/{file_path}: {e}", exc_info=True)
    ...
    def delete_file(
        ...
        try:
            ...
            logger.error(f"Failed to delete file from {bucket}/{file_path}: {e}", exc_info=True)
            if "not found" in str(e).lower() or "404" in str(e):
                raise FileNotFoundError(f"File not found: {bucket}/{file_path}")
    ...
    def list_files(
        ...
        try:
            ...
            for file_info in files:
                if isinstance(file_info, dict):
                    result.append({
                        ...
                        "path": f"{folder_path}/{file_info.get('name')}" if folder_path else file_info.get("name"),
            ...
            logger.error(f"Failed to list files in {bucket}/{folder_path}: {e}", exc_info=True)
    ...
    def get_public_url(
        ...
        try:
            ...
            logger.error(f"Failed to get public URL for {bucket}/{file_path}: {e}", exc_info=True)
    ...
    def create_signed_url(
        ...
        try:
            ...
            logger.error(f"Failed to create signed URL for {bucket}/{file_path}: {e}", exc_info=True)
    ...
    def _ensure_bucket_exists(self, bucket: str) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Bucket '{bucket}' may not exist. Create it in Supabase Dashboard: Storage > New Bucket")
            except Exception as e:
                logger.warning(f"Could not verify/create bucket '{bucket}': {e}. Ensure bucket exists in Supabase Dashboard.")
