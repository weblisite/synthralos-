...
from app.models import OSINTStream, OSINTAlert, OSINTSignal
...
class OSINTService:
    ...
    def _initialize_engines(self) -> None:
        ...
        self._engines.setdefault("social_listener", {"name": "social_listener", "is_available": False})
        self._engines.setdefault("newscatcher", {"name": "newscatcher", "is_available": False})
    ...
    def create_stream(
        ...
        logger.info(f"Created OSINT stream: {osint_stream.id} (Platform: {platform}, Engine: {engine})")
    ...
    def create_alert(
        ...
        logger.info(f"Created OSINT alert: {alert.id} (Type: {alert_type}, Severity: {severity})")
    ...
    def check_stream_alerts(
        ...
        recent_signals = session.exec(
            select(OSINTSignal).where(
                OSINTSignal.stream_id == stream.id
            ).order_by(OSINTSignal.created_at.desc()).limit(100)
        ).all()
    ...
    def _execute_tweepy(
        ...
        try:
            ...
            if auth_type == "bearer":
                ...
                if query_type == "search":
                    tweets = client.search_tweets(q=query_text, count=10, tweet_mode="extended")
                    for tweet in tweets:
                        items.append({
                            "id": tweet.id,
                    ...
                    tweets = client.user_timeline(screen_name=query_text, count=10, tweet_mode="extended")
                    for tweet in tweets:
                        items.append({
                            "id": tweet.id,
                            "text": tweet.full_text if hasattr(tweet, "full_text") else tweet.text,
