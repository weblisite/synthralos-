...
class CodeExecutionService:
    ...
    def create_execution(
        ...
        logger.info(f"Created code execution: {execution.id} (Runtime: {runtime}, Language: {language})")
    ...
    def execute_code(
        ...
        if execution.status != "running":
            raise CodeExecutionError(f"Code execution {execution_id} is not in running status")
        ...
        try:
            ...
            logger.info(f"Executed code: {execution_id} (Duration: {duration_ms}ms, Exit Code: {execution.exit_code})")
    ...
    def create_sandbox(
        ...
        logger.info(f"Created code sandbox: {sandbox.id} (Runtime: {runtime}, Name: {name})")
    ...
    def _execute_with_subprocess(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=self._get_file_extension(language)) as f:
                f.write(code)
                temp_file = f.name
            ...
            try:
                ...
                # Execute code
                start_memory = resource.getrusage(resource.RUSAGE_CHILDREN).ru_maxrss / 1024  # KB to MB
                ...
                stdin_data = json.dumps(input_data.get("input", {})) if input_data.get("input") else None
                ...
                end_memory = resource.getrusage(resource.RUSAGE_CHILDREN).ru_maxrss / 1024
