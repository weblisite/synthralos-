...
class WazuhClient:
    ...
    def __init__(self):
        ...
        if not self.is_available:
            logger.warning(
                "Wazuh not configured (WAZUH_URL not set). Security monitoring will be disabled."
            )
            logger.info(
                "To enable Wazuh: Set WAZUH_URL environment variable. See docs/OBSERVABILITY_SETUP.md"
            )
            self.wazuh_url = None
