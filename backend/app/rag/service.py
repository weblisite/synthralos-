...
class RAGService:
    ...
    def _initialize_chromadb(self) -> None:
        ...
        if not settings.CHROMA_SERVER_HOST:
            logger.info(
                "ChromaDB not configured (CHROMA_SERVER_HOST not set). Using placeholder client."
            )
            logger.info(
                "To enable ChromaDB: Set CHROMA_SERVER_HOST environment variable. See docs/OBSERVABILITY_SETUP.md"
            )
            return
        ...
        try:
            ...
            chroma_client = chromadb.HttpClient(
                ...
                headers={"Authorization": f"Bearer {settings.CHROMA_SERVER_AUTH_TOKEN}"}
                if settings.CHROMA_SERVER_AUTH_TOKEN
                else None,
            )
            ...
            logger.info(
                f"✅ ChromaDB client initialized ({settings.CHROMA_SERVER_HOST}:{settings.CHROMA_SERVER_HTTP_PORT})"
            )
            ...
            logger.warning(
                f"Failed to initialize ChromaDB client: {e}. Using placeholder."
            )
    ...
    def _get_vector_db_client(self, vector_db_type: str) -> Any:
        ...
        client = self._vector_db_clients.get(
            vector_db_type,
            {
                "type": vector_db_type,
                "is_available": False,
        ...
        if not client.get("is_available"):
            logger.warning(
                f"Vector DB '{vector_db_type}' not available. Using placeholder."
            )
            ...
            try:
                ...
                if not query_results:
                    return self._execute_database_query(
                        session, index_id, query_text, top_k
                    )
                ...
                logger.warning(
                    f"ChromaDB query failed: {e}. Falling back to database query."
                )
                return self._execute_database_query(
                    session, index_id, query_text, top_k
                )
    ...
    def create_index(
        ...
        logger.info(
            f"Created RAG index: {name} (ID: {index.id}, Vector DB: {vector_db_type})"
        )
    ...
    def add_document_from_storage(
        ...
        try:
            ...
            logger.error(
                f"Failed to add document from storage {bucket}/{storage_path}: {e}",
                exc_info=True,
            )
    ...
    def add_document(
        ...
        if index.vector_db_type == "chromadb":
            try:
                ...
                if (
                    chroma_client
                    and chroma_client.get("is_available")
                    and "client" in chroma_client
                ):
                    collection_name = f"rag_index_{index_id}"
                    try:
                        collection = chroma_client["client"].get_collection(
                    ...
                    logger.info(
                        f"Synced document {document.id} to ChromaDB collection {collection_name}"
                    )
            except Exception as e:
                logger.warning(
                    f"Failed to sync document to ChromaDB: {e}. Document saved to database only."
                )
