...
class ExecutionState(BaseModel):
    ...
    execution_data: dict[str, Any] = Field(default_factory=dict)  # Data passed between nodes
    started_at: datetime
...
class WorkflowState(BaseModel):
    ...
    workflow_id: UUID
    version: int
    nodes: dict[str, dict[str, Any]] = Field(default_factory=dict)  # node_id -> node config
    edges: list[dict[str, str]] = Field(default_factory=list)  # [{"from": "node1", "to": "node2"}]
