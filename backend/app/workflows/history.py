...
class ExecutionHistory:
    ...
    def get_execution_summary(
        ...
        completed_nodes = len(
            [r for r in state.node_results.values() if r.status == "success"]
        )
        failed_nodes = len(
            [r for r in state.node_results.values() if r.status == "failed"]
        )
    ...
    def replay_execution(
        ...
        if from_node_id:
            ...
            self.workflow_engine.save_execution_state(
                session, new_execution.id, new_state
            )
