...
class LangGraphEngine:
    ...
    def build_graph(
                ...
                def node_func(state: dict[str, Any]) -> dict[str, Any]:
                    ...
                    if handler:
                        ...
                        if result.status == "success":
                            ...
                            state["last_result"] = {"status": "success", "output": result.output}
                        else:
                            state["last_result"] = {"status": "failed", "error": result.error}
                ...
                if edge.get("condition"):
                    ...
                    def make_condition_func(condition_expr: str):
                        def condition_func(state: dict[str, Any]) -> str:
                            ...
                            if condition_expr == "success" and last_result.get("status") == "success":
                                return to_node
                            elif condition_expr == "failed" and last_result.get("status") == "failed":
                                return to_node
                            return self.END
        ...
        for node_id in workflow_state.nodes.keys():
            outgoing_edges = [e for e in workflow_state.edges if e.get("from") == node_id]
