...
class TimingRandomizer:
    ...
    def human_like_delay(
        self, base_seconds: float = 2.0, variance: float = 0.5
    ) -> None:
        """
...
class FingerprintSpoofer:
    ...
    def get_headers(
        self, user_agent: str, fingerprint: dict[str, Any] | None = None
    ) -> dict[str, str]:
        """
...
class StealthService:
    ...
    def get_stealth_headers(self) -> dict[str, str]:
        ...
        """
        user_agent = (
            self.ua_rotator.get_user_agent()
            if self.config.enable_ua_rotation
            else self.ua_rotator._user_agents[0]
        )
        ...
        return self.fingerprint_spoofer.get_headers(
            user_agent, self._current_fingerprint
        )
    ...
    def get_click_preparation_path(
