...
class Proxy:
    ...
    def get_proxy_url(self) -> str:
        ...
        if self.username and self.password:
            return (
                f"http://{self.username}:{self.password}@{self.ip_address}:{self.port}"
            )
...
class ProxyPoolManager:
    ...
    def add_proxy(
        ...
        logger.info(
            f"Added proxy to pool: {proxy_id} ({proxy_type}, {country or 'unknown'})"
        )
        return proxy
    ...
    def get_proxy(
        ...
        available_proxies.sort(
            key=lambda p: (
                ...
                p.last_used_at
                if p.last_used_at
                else datetime.min,  # Least recently used first
            )
        )
    ...
    def _update_proxy_score(
        ...
        successful_requests = sum(
            1 for log in logs if log.status_code and 200 <= log.status_code < 300
        )
        failed_requests = sum(
            1 for log in logs if log.status_code and log.status_code >= 400
        )
        blocked_requests = sum(1 for log in logs if log.block_reason is not None)

        avg_latency = (
            sum(log.latency_ms for log in logs) / total_requests
            if total_requests > 0
            else 0
        )
        ...
        success_rate_score = (
            (successful_requests / total_requests * 100) if total_requests > 0 else 50
        )
        latency_score = max(0, 100 - (avg_latency / 10))  # Lower latency = higher score
        block_rate_score = (
            max(0, 100 - (blocked_requests / total_requests * 100))
            if total_requests > 0
            else 50
        )
        ...
        # Weighted score
        proxy.score = (
            success_rate_score * 0.4 + latency_score * 0.3 + block_rate_score * 0.3
        )
        ...
        if proxy.score < 20:
            ...
            logger.warning(
                f"Deactivated proxy {proxy.proxy_id} due to low score: {proxy.score:.2f}"
            )
    ...
    def deactivate_proxy(self, proxy_id: str, reason: str | None = None) -> None:
        ...
        if proxy:
            proxy.is_active = False
            logger.info(
                f"Deactivated proxy {proxy_id}: {reason or 'Manual deactivation'}"
            )
    ...
    def get_proxy_statistics(
        ...
        return {
            ...
            "last_used_at": proxy.last_used_at.isoformat()
